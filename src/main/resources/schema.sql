...
CREATE TABLE IF NOT EXISTS tbl_dm_rooms (
                                            ...
                                            user1_id UUID REFERENCES tbl_users(id) ON DELETE SET NULL, -- 탈퇴 시 NULL
                                            user2_id UUID REFERENCES tbl_users(id) ON DELETE SET NULL, -- 탈퇴 시 NULL
...
CREATE TABLE IF NOT EXISTS tbl_dm_messages (
                                               id UUID PRIMARY KEY,
                                               room_id UUID NOT NULL REFERENCES tbl_dm_rooms(id) ON DELETE CASCADE,
                                               sender_id UUID REFERENCES tbl_users(id) ON DELETE SET NULL,
                                               content TEXT NOT NULL,
                                               created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
                                               read_at TIMESTAMPTZ
);
