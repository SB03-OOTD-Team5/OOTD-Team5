 ...
 * - keywords: 이름/속성 파싱용
 * - forceInclude(info): 특정 조건에서 무조건 포함
 * - probability(info, base): 그 외 확률(0.0~1.0)
 */
public enum OptionalItemSubType {
    ...
    public boolean shouldInclude(RecommendationInfoDto info) {
        if (info == null || info.weatherInfo() == null || this == NONE) return false;
        ...
        return switch (this) {
            case UMBRELLA ->
                precip.isRainy() || precip.isSnowy() || rainProb >= 0.6;

            case SUNSHADE ->
                precip.isClear() && rainProb < 0.2 && feels >= 24
                    && wind != WindspeedLevel.STRONG;

            case FAN ->
                feels >= 27 && precip.isClear() && rainProb < 0.2
                    && wind != WindspeedLevel.STRONG;
