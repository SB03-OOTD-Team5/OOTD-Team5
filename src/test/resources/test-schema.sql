...
(
    ...
    CONSTRAINT check_precipitation_type CHECK (precipitation_type IN
...
/* 인덱스 설정 */
-- tbl_weathers index
CREATE INDEX IF NOT EXISTS idx_tbl_weathers_profile_forecasted_at
    ON tbl_weathers (profile_id, forecasted_at);

CREATE INDEX IF NOT EXISTS idx_tbl_weathers_lat_lon
    ON tbl_weathers (latitude, longitude);

-- tbl_locations index
CREATE INDEX IF NOT EXISTS idx_tbl_locations_lat_lon
    ON tbl_locations (latitude, longitude);

-- tbl_clothes index
CREATE INDEX IF NOT EXISTS idx_tbl_clothes_owner_id
